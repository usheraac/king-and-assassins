...
class KingAndAssassinsClient(game.GameClient):
    ...
    def _nextmove(self, state):
        ...
        def randommove(position): 
            card=["N","E","W","S"]
            cardp= random.choice(card)
            x,y=position
            move="move",x,y,cardp
            return move
                ...
                def Mouv(Ap):
                    ...
                    for i in range(len(L)):
                        for j in range(len(L[i])):
                            if L[i][j] in POPULATION : #recupère position des citoyens
                                population.append((i,j))
                            if L[i][j]=='knight':#position de chevalier
                                knights.append((i,j))
                            if L[i][j]=='king':#position du Roi
                                king.append((i,j))
                    c=0
                    movepool=[]
                    while c<Ap:

                        movepool.append(randommove(random.choice(population)))
                        c+=1
                    return movepool
                return json.dumps({'actions': Mouv(state["card"][3])}, separators=(',', ':'))
            else:
                def Mouv(Ap):
                    i=0
                    j=0
                    ...
                    while c<Ap:
                        ...
                        movepool.append(randommove(random.choice(knights)))
                        c+=1
                    movepool.append(randommove(random.choice(king)))
                    return movepool
                return json.dumps({'actions': Mouv(state["card"][1])}, separators=(',', ':'))
